...
- Onboards the iPhone by manual entry or a private setup QR generated by the
  installer.
- Installs the recommended Linux server from the latest verified release with
  one `curl` command—no repository clone or local build required.
- Ships native macOS and Windows server bundles in addition to the managed
  Linux service.
...
The Linux installer remains the recommended always-on deployment because it
...
Download the `darwin-amd64` or `darwin-arm64` release, extract it, and run:

```bash
./start-server.sh
```

The launcher finds `codexbar`/`CodexBarCLI`, creates a random bearer token, and
stores configuration plus SQLite data in
`~/Library/Application Support/UsageWidget`. Keep the terminal open. For a
source checkout, build first with
...
Download and extract the `windows-amd64` release, then in PowerShell run:

```powershell
powershell -ExecutionPolicy Bypass -File .\start-server.ps1 `
  -CodexBarUrl http://YOUR-CODEXBAR-HOST:8765/usage
```

The server and SQLite database are native Windows binaries/files. Configuration
is stored under `%LOCALAPPDATA%\UsageWidget`. CodexBar currently publishes
