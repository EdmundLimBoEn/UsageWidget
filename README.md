...
- Onboards the iPhone by manual entry or a private setup QR generated by the
  installer.
- Installs the latest verified Linux, macOS, or Windows release from one hosted
  installer service—no repository clone or local build required.
...
The hosted installer supports all three operating systems and both release
architectures. Linux and macOS share the Unix command; Windows uses its native
...
Both commands prompt interactively for required values, resolve the latest
GitHub release, verify its SHA-256 checksum, and select amd64 or arm64 without
flags. Existing configuration and database files live outside the replaceable
application directory and are retained on updates.
...
session and invokes `sudo` only for the system installation steps. You do not
...
Run the same no-flag Unix installer used on Linux:

```bash
curl -fsSL https://usagewidget.edmundlim.systems/install.sh | bash
```

It detects Intel or Apple silicon, verifies and installs the matching release
under `~/Library/Application Support/UsageWidget/App`, then starts the native
server. The installer finds `codexbar`/`CodexBarCLI` or asks for a private
CodexBar usage URL, while the launcher creates a random bearer token. Saved
...
In PowerShell, run the native no-flag installer:

```powershell
irm https://usagewidget.edmundlim.systems/install.ps1 | iex
```

It detects x64 or ARM64, verifies and installs the matching native bundle under
`%LOCALAPPDATA%\UsageWidget\App`, asks for a private CodexBar usage URL when no
compatible CLI or saved configuration exists, and starts the server. The
